 ...
 */
@Stateless
public class Notesdeselevesseq4Facade extends AbstractFacade<Notesdeselevesseq4> implements Notesdeselevesseq4FacadeLocal {
    ...
    public Notesdeselevesseq4Facade() {
        super(Notesdeselevesseq4.class);
    }

    @Override
    public Integer nextId() {
        try {
            Query query = em.createNamedQuery("Notesdeselevesseq4.nexId");
    ...
    @Override
    public List<Notesdeselevesseq4> listeMatiereClasse(String codeMatiere, String codeClasse) {
        try {
            Query query = em.createNamedQuery("Notesdeselevesseq4.findMatiereClasse");
            query.setParameter("codematiere", codeMatiere);
            query.setParameter("codeclasse", codeClasse);
            List<Notesdeselevesseq4> list = query.getResultList();
            if (list.isEmpty()) {
                return null;
            } else {
                return list;
    ...
    public List<Listedeseleves> listeDesNouveauxEleves(String codeMatiere, String codeclasse) {
        try {
            Query query = em.createNamedQuery("Notesdeselevesseq4.findNouveauxEleves");
            query.setParameter("codematiere", codeMatiere);
            query.setParameter("codeclasse", codeclasse);
...
//calcul moyenne de la classe
    ...
    public BigDecimal findMoyenne(String codeMatiere, String codeclasse) {
        try {
            Query query;
            query = em.createNamedQuery("Notesdeselevesseq4.findAll2");
            query.setParameter("codeclasse", codeclasse);
            query.setParameter("codematiere", codeMatiere);
            List<Notesdeselevesseq4> eleves_de_la_classe = query.getResultList();
            Iterator i = eleves_de_la_classe.iterator();
            Notesdeselevesseq4 el;
            BigDecimal sum = BigDecimal.valueOf(0);
            while (i.hasNext()) {
                el = (Notesdeselevesseq4) i.next();
    ...
    public BigDecimal findGandeMoyenne(String codeMatiere, String codeclasse) {
        Query query;
        query = em.createNamedQuery("Notesdeselevesseq4.findMoyenneMax");
        query.setParameter("codeclasse", codeclasse);
        query.setParameter("codematiere", codeMatiere);
    ...
    public BigDecimal findPetiteMoyenne(String codeMatiere, String codeclasse) {
        Query query;
        query = em.createNamedQuery("Notesdeselevesseq4.findMoyenneMin");
        query.setParameter("codeclasse", codeclasse);
        query.setParameter("codematiere", codeMatiere);
    ...
    public BigDecimal findTauxreussite(String codeMatiere, String codeclasse) {
        try {
            Query query;
            Query query1;
            query = em.createNamedQuery("Notesdeselevesseq4.findTauxReussite");
            query.setParameter("codeclasse", codeclasse);
            query.setParameter("codematiere", codeMatiere);
            query.setParameter("lanote", BigDecimal.valueOf(10));
            List<Notesdeselevesseq4> effectif_ayant_la_moyenne = query.getResultList();
            query1 = em.createNamedQuery("Notesdeselevesseq4.findAll2");
            query1.setParameter("codeclasse", codeclasse);
            query1.setParameter("codematiere", codeMatiere);
            List<Notesdeselevesseq4> effectif_tottal = query1.getResultList();
    ...
    public BigDecimal noteEleve(String matriculeEleve, String codeMatiere) {
        Query query = em.createNamedQuery("Notesdeselevesseq4.findNoteEleve");
        query.setParameter("matricule", matriculeEleve);
        query.setParameter("codematiere", codeMatiere);
        Notesdeselevesseq4 list = (Notesdeselevesseq4) query.getSingleResult();
    ...
    public int eleveAbsent(String codeMatiere, String codeclasse) {
        Query query = em.createNamedQuery("Notesdeselevesseq4.findAll1");
        query.setParameter("codeclasse", codeclasse);
        query.setParameter("codematiere", codeMatiere);
        List<Notesdeselevesseq4> list = query.getResultList();
        if (list == null) {
    ...
    public int eleveComposé(String codeMatiere, String codeclasse) {
        Query query = em.createNamedQuery("Notesdeselevesseq4.findAll2");
        query.setParameter("codeclasse", codeclasse);
        query.setParameter("codematiere", codeMatiere);
        List<Notesdeselevesseq4> list = query.getResultList();
        if (list == null) {
    ...
    public boolean existEleve_et_sa_matiere(String matriculeEleve, String codeMatiere) {
        try {
            Notesdeselevesseq4 n = em.createNamedQuery("Notesdeselevesseq4.findNoteEleve", Notesdeselevesseq4.class).setParameter("matricule", matriculeEleve).setParameter("codematiere", codeMatiere).getSingleResult();
            if (n != null) {
                return true;
            } else {
                return false;
    ...
    @Override
    public List<Notesdeselevesseq4> Eleve_et_sa_matiere(String matriculeEleve) {
        Query query = em.createNamedQuery("Notesdeselevesseq4.findByMatriculeeleve");
    ...
    @Override
    public List<Object[]> bulletinSeq4(String codeclass) {
        try {
    ...
    @Override
    public List<Object[]> bulletinSeq4ParEleve(String codeclass, String matriculeEleve) {
        try {
            Query query = em.createNativeQuery("select c.codeclasse,\n"
                    + "l.matriculeeleve,concat(l.nom,' ',l.prenom) as nomeleve ,l.sexe,l.datenaiss,l.lieunaiss,l.adresse,l.redoublant,l.photo,\n"
                    + "cl.codematiere,cl.coefficient,cl.groupe,\n"
...
//bulletin pour un élève séquence 1
    ...
    @Override
    public List<Object[]> bulletinSeq4ParSerie(String codeclass, String matriculeEleve, String matriculeEleveA) {
        try {
            Query query = em.createNativeQuery("select c.codeclasse,\n"
                    + "l.matriculeeleve,concat(l.nom,' ',l.prenom) as nomeleve ,l.sexe,l.datenaiss,l.lieunaiss,l.adresse,l.redoublant,l.photo,\n"
                    + "cl.codematiere,cl.coefficient,cl.groupe,\n"
    ...
    @Override
    public List<Object[]> pvSeq4(String codeclass) {
        try {
    ...
    @Override
    public List<Object[]> synthSeq4(String codeclass) {
        try {
    ...
    public List<Object[]> effG_F8_par_interval(String codeclass) {
        try {
            Query query = em.createNativeQuery("SELECT \n"
                    ...
                    + "(select count(matriculeeleve) from seq4moyennesrangsdeseleves where moyenne between 18 and 20 and matriculeeleve in (select matriculeeleve from listedeseleves where codeclasse=?18 and sexe='G')) as eff18_20");
            query.setParameter(1, codeclass);
            query.setParameter(2, codeclass);
            query.setParameter(3, codeclass);
            query.setParameter(4, codeclass);
    ...
    public List<Object[]> Moy_premier_Dernier(String codeclass) {
        try {
            Query query = em.createNativeQuery("select moyenne,moyennepremier,moyennedernier from calculnotesdesclasses where codeclasse=?1 and periode ='SEQ4' order by periode");
            query.setParameter(1, codeclass);
            List<Object[]> falist = query.getResultList();
            if (falist.isEmpty()) {
                return null;
    ...
    @Override
    public List<Object[]> noteElevesSeq4(String codeclass) {
        try {
